...
partial model Material_Simple_InFlow_ResTime_Base
  ...
  // Input parameters
  inner parameter Concentration[specRec.nSpec] c "initial concentration in mol/m^3";
  inner parameter Temperature T "Temperature of the input flow, EBRec.T0convectiveInflow or CondRec.T0";
  ...
  parameter Velocity v = GeoRec.Z/t "velocity of gas for given time in electrolyzer, whole length z";
  parameter VolumeFlowRate V_flow = A*v "in m^3/s";

  // variables
  MolarFlowRate molFlow "in mol/s";
...
equation
  ...
  for i in 1:specRec.nSpec loop
    convFlow.molFlow_vec[i] = - molFlow * c[i]/sum(c);
  end for;
